...
public abstract class BasicGameFrame extends JFrame implements ActionListener, IBoardGameUI {
	...
	protected BasicGameFrame() throws BoardGameException {
		...
		mainPanel.add(new JScrollPane(hostedGameArea, ScrollPaneConstants.VERTICAL_SCROLLBAR_AS_NEEDED,
		...
		SwingUtils.createButton(Message.getMessage("action.pass"), CMD_PASS, commonCmdPanel, this, true);
		SwingUtils.createButton(Message.getMessage("action.advise"), CMD_ADVISE, commonCmdPanel, this, true);
		SwingUtils.createButton(Message.getMessage("action.undo"), CMD_UNDO, commonCmdPanel, this, true);
		...
		SwingUtils.addMenuItem(Message.getMessage("action.setup_players"), CMD_SETUP_PLAYERS, this, menuOptions);
	...
	public final void actionPerformed(ActionEvent e) {
		...
		if (command != null) {
			...
			if (command.equals(CMD_NEW_GAME)) {
				...
				displayMessage(SwingUtils.MessageType.ERROR, Message.getMessage("command.unknown", command));
	...
	public void setStatus(String status) {
		...
		gameStatusLabel.setText(status + "|" + Message.getMessage("info.free_mem", free / BIARY_KILO_UNIT_MULTIPLIER));
	}
	...
	public void saveGame() {
		...
		if (fileChoosingResult == JFileChooser.APPROVE_OPTION) {
			try {
				...
				displayMessage(SwingUtils.MessageType.ERROR, "Unable to save the game :\n" + e.getMessage());
			}
		}
	}
