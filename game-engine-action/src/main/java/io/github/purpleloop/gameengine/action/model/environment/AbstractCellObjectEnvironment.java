...
public abstract class AbstractCellObjectEnvironment extends AbstractObjectEnvironment {
    ...
    /**
     * Maximum number of tries before giving up in searches. Should be
     * sufficiently large to allow dispersion and sufficiently small to prevent
     * blocking.
     */
     ...
     */
    protected AbstractCellObjectEnvironment(ISession session, IGameLevel level)
    ...
    protected void initStorage(int cellWidth, int cellHeight) {
        ...
        LOG.debug("Creating the environment storage of " + cellWidth + " x " + cellHeight
                + " cells => " + width + " x " + height + " base units");
     ...
     * @param cx abscissa of the cell
     * @param cy ordinate of the cell
     * @return true if the location (cx, cy) are the coordinates of a valid cell
     *         of the environment, false otherwise
     */
    public boolean isValidCell(int cx, int cy) {
     ...
     * @return true if the location is in bounds (for a standard object
     *         occupying one cell), false otherwise
     */
     ...
     * @param x abscissa in base units
     * @param y ordinate in base units
     * @return true if the given object is allowed to be at the given location
     *         (x, y) of the environment, false otherwise
     */
    public boolean isObjectAllowedAtLocation(IEnvironmentObjet object, int x, int y) {
        ...
        return isObjectAllowedAtCell(object, cx1, cy1) && isObjectAllowedAtCell(object, cx2, cy1)
                && isObjectAllowedAtCell(object, cx1, cy2)
                && isObjectAllowedAtCell(object, cx2, cy2);
    }
     ...
     * @param cx abscissa of the cell
     * @param cy ordinate of the cell
     * @return true if the given object is allowed to be at the given cell (cx,
     *         cy) of the environment, false otherwise
     */
     ...
     */
    public Optional<Location> findFirstCellLocationMatchingContents(ICellContents contents) {
    ...
    /**
     * Search for the all cells location having the given contents.
     * 
     * @param contents cell content to match
     * @return list of all locations matching the requested contents
     */
    public List<Location> findAllCellsLocationsMatchingContents(ICellContents contents) {
        ...
        for (int y = 0; y < cellHeight; y++) {
            for (int x = 0; x < cellWidth; x++) {
                if (getCellContents(x, y).equals(contents)) {
                    locations.add(Location.getLocation(x, y));
                }
            }
        }
        return locations;
