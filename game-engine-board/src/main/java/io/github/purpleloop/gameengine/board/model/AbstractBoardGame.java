...
public abstract class AbstractBoardGame implements Runnable {
	 ...
	 */
	public final synchronized void setPlayers(List<IPlayerInfo> playerInfos) throws BoardGameException {
	 ...
	 */
	public final IPlayerRole getRoleForPlayer(IPlayerInfo player) {
	...
	public final boolean handlePlay(IBoardPlay play) {
		...
		if (!isHumanTurn()) {
			gameUi.displayMessage(SwingUtils.MessageType.ERROR, Message.getMessage("error.not_your_turn"));
			return false;
		}
	 ...
	 *         otherwise.
	 */
	protected abstract boolean doHandlePlay(IBoardPlay play);
	...
	public final void run() {
		...
		gameUi.displayMessage(SwingUtils.MessageType.INFO, "The game has ended : " + board.getWinningStatus());
	}
	 ...
	 */
	protected abstract void cpuPlay(IPlayerInfo currentPlayer);
	...
	public final void newGame() throws BoardGameException {
		...
		if (playersCount < minPlayers || playersCount > maxPlayers) {
			throw new BoardGameException(Message.getMessage("error.players.count", minPlayers, maxPlayers));
		}
		...
		if (sayWhoStarts.isEmpty()) {
			throw new BoardGameException("Unable to determine the first player that can start the game.");
		}
	...
	/**
	 * This method allows to determines which player must start. It depends on the
	 * concrete game. It can depend on the players themselves (the youngest start) or
	 * of the board accessories (roll a dice).
	 * 
	...
	protected final Optional<IPlayerInfo> getPlayerForRole(IPlayerRole role) {
		return roleMap.entrySet().stream().filter(entry -> entry.getValue().equals(role)).map(Map.Entry::getKey)
				.findFirst();
	}
